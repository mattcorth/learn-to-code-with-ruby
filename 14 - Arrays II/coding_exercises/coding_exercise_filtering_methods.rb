...
def reverse_all(strArr)
    strArr.map { |value| value.reverse }
end
...
def words_with_letter(strArr, letter)
    strArr.select { |value| value.include? letter }
end
...
def evens_and_odds(numArr)
    trueArr, falseArr = numArr.partition { |value| value.even?}
    [trueArr, falseArr]
end
